    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] strings) {
        if (command.getName().equalsIgnoreCase("wasteutils")) {
            if (strings[0].equalsIgnoreCase("reload")) {
                ...
                } else if (strings[0].equalsIgnoreCase("mail")) {
                    if (sender.hasPermission("wasteutils.mail")) {
                        sender.sendMessage(this.plugin.addPrefix(this.plugin.lang.getString("mail.test")));
                        this.plugin.loadConfig();
                    } else {
